...
public static class UninstallRules
{
    ...
    public static bool IsProtected(string path)
    {
        ...
        var relay = Path.GetDirectoryName(Environment.ProcessPath ?? "") ?? "";
        var appData = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Relay");
        var local = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Relay");
        return full.StartsWith(windows, StringComparison.OrdinalIgnoreCase)
               || (relay.Length > 0 && full.StartsWith(relay, StringComparison.OrdinalIgnoreCase))
