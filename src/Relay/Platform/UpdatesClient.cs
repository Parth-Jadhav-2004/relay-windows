...
internal static class UpdatesClient
{
    ...
    public const string Repo = "relay-windows";
    ...
    public static void LaunchInstaller(string payloadDir)
    {
        ...
        var exe = Path.Combine(dest, "Relay.exe");
        ...
        {
            FileName = "cmd.exe",
            Arguments = "/d /c start \"RelayUpdate\" /min cmd.exe /d /c \"" + bat + "\"",
    ...
    static HttpClient CreateClient()
    {
        ...
        client.DefaultRequestHeaders.TryAddWithoutValidation("User-Agent", "Relay-Windows/" + InstalledLabel);
    ...
    {
        if (File.Exists(Path.Combine(stagingRoot, "Relay.exe")))
            return stagingRoot;
        var nested = Directory.GetDirectories(stagingRoot);
        if (nested.Length == 1 && File.Exists(Path.Combine(nested[0], "Relay.exe")))
            return nested[0];
        throw new InvalidOperationException("The update zip did not contain Relay.exe.");
    }
}
